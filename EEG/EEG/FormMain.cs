...
namespace EEG
{
    ...
    public partial class FormMain : Form
    {
        ...
        private void Data_Processing(int Frequency)
        {
            ...
            int Data_Count = 0;
            ///Параметр определяющий децимацию
            int Decimal_Count = 0;
            short x;
            ...
            while (Started == 1)
            {
                ...
                for (int J = 0; (J < Frame.CountChannels) && (Started == 1); J++)
                {
                    ...
                    if (Started == 1)
                    {
                        ...
                        if (!chFilterCC.Checked || (CountCC[J] == CC[J].Length))
                        {
                            if (!chDrawDisable.Checked && Decimal_Count == 0)
                                DrawBuf[J].AddLast(x);
                            Decimal_Count = (Decimal_Count + 1) % Decimal;
        ...
        void Client_OnReseive(object sender, EventClientMsgArgs e)
        {
            ...
            if ((Math.Abs(N) == 1) || (Math.Abs(N) == 2))
            {
                ...
                while ((I >= 0) && ((chDestinations.Items[I] as ClientAddress).Id != M))
                    I--;
                if (I >= 0)
        ...
        private string FileName
        {
            get
            {
                return "eeg(" + DateTime.Now.ToString().Replace(':', '.') + ").dat";
            }
        }
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    chSave.Enabled = true;
                    chCoefCC.Enabled = chFilterCC.Enabled;
                    numberFilterCC.Enabled = chFilterCC.Enabled;
                    chFilterCC.Enabled = true;
                    btnCoefCC.Enabled = chCoefCC.Enabled;
                    dgFilterCC.Enabled = chCoefCC.Enabled;
        ...
        private void panelDraw_Paint(object sender, PaintEventArgs e)
        {
            ...
            try
            {
                ...
                if (Count > 0)
                {
                    ...
                    float ScaleX = (float)nScaleX.Value * Decimal;
        ...
        {
            for (int I = 0; I < DrawBuf.Length; I++)
                DrawBuf[I].Capacity = (int)Math.Ceiling((panelDraw.ClientRectangle.Width - ShiftX - 10) / Math.Max(nScaleX.Value, 1));
            Decimal = (int)Math.Max(1 / nScaleX.Value, 1);
        }
