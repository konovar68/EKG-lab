...
namespace NetManager
{
    ...
    class NMClient
    {
        ...
        /// <summary>
        /// <summary>
        /// Задерживает поток до тех пор, пока в буфере tcp клиента не будет нужное число байт данных 
        /// или tcp клиент не будет отключен
        /// </summary>
        /// <param name="size">Число ожидаемых байт</param>
        /// <returns>массив байт</returns>
        private byte[] Wait(int size)
        {
            ...
            while (offset < size)
            {
                len = Math.Min(Client.Available, size - offset);
                if (len > 0)
                {
                    ns.Read(res, offset, len);
                    offset += len;
                }
                else
                    Thread.Yield();
        ...
        private void Run_Client()
        {
            try
            {
                ...
                {
                    int N = BitConverter.ToInt32(Wait(sizeof(int)), 0);
                    if (Running && Client.Connected)
                    {
                        if (N == 0)//обработка сообщений от сервера
                        {
                            N = BitConverter.ToInt32(Wait(sizeof(int)), 0);
                            if (Running && Client.Connected)
                            {
                                if (N != 0)//код команды
                                {
                                    if (N != (int) 0x7FFFFFFF)
                                    {
                                        int Id = BitConverter.ToInt32(Wait(sizeof(int)), 0);
                                        if (Running && Client.Connected)
                                        {
                                            int Len = BitConverter.ToInt32(Wait(sizeof(int)), 0);
                                            if (Running && Client.Connected)
                                            {
                                                byte[] buf = Wait(Len * sizeof(char));
                                                if (Running && Client.Connected)
                                                {
                                                    string S = "";
